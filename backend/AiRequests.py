...
client = openai.OpenAI(
    ...
    timeout=30.0 # Уменьшаем таймаут для более быстрой реакции
)
...
def request_ai(prompt, user_msg, msg_type, msg_history=None, thread_id=None):
    try:
        ...
        # --- НОВЫЙ, СТАНДАРТНЫЙ ВЫЗОВ API ---
        response = client.chat.completions.create(
            model="gpt-4o-mini", # Используем более новую, быструю и дешевую модель
            messages=messages,
            tools=functions,
        ...
        result = {
            "text": text_content,
            "type": 'message',
            "links": [] # Аннотаций здесь больше не будет
        }
        return {"result": result}
